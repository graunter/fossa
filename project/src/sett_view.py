...
class SettingsFrame(ttk.TTkFrame):
    ...
    def __init__(self, *,
        ...
        tax_tbl = [
            ['Time', 'Workday', 'Holiday', 'Saturday', 'Sunday']
            #, ['Time', 'Workday', 'Holiday', 'Saturday', 'Sunday']            
            #, ['a', 'b', 'c'] for i in range(16)
            #, ['a', 'b', 'c', 'd', 'e'] for i in range(16)
        ]

        for _ in range(3):
            tax_tbl.append(['a', 'b', 'c', 'd', 'e', 'f', 'g'])

        tax_tableModel = ttk.TTkTableModelList(data=tax_tbl)        
        ...
        self.get_tax_btn.clicked.connect(self.on_read_tax)
        ...
        self.layout().addWidget(ttk.TTkLabel(text="      Time      Workday    Holiday    Saturday    Sunday  ", maxHeight = 1))
        self.layout().addWidget(self.tax_table)

    def on_read_tax(self):
        self.on_upd()
